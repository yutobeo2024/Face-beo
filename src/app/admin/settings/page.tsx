...
/** Cấu hình → Zalo OA: trạng thái kết nối, các nhóm nhận tin theo loại tin, gửi tin thử, 10 tin nhóm gần nhất. */
function ZaloCard({ busy, run }: { busy: boolean; run: (fn: () => Promise<unknown>, ok: string, after?: () => void) => Promise<void> }) {
  ...
  return (
    <Card>
      <CardHeader title="Zalo OA — nhóm nhận tin" actions={<IconButton icon="refresh" label="Tải lại" onClick={reload} />} />
      {!d ? (
        <div className="p-4">
          <Loading />
        </div>
        ...
        <div className="space-y-3 p-4 sm:p-5">
          ...
          <div>
            <p className="mb-1 text-sm font-semibold text-slate-700">Nhóm và loại tin nhận</p>
            ...
            {groups.data?.groups.length ? (
              <ul className="mb-2 space-y-2">
                {groups.data.groups.map((g) => (
                  <ZaloGroupItem key={`${g.groupId}:${g.categories.join()}:${g.departmentIds.join()}`} g={g} depts={deptList} busy={busy} run={run} onSaved={reload} onTest={() => sendTest(g.groupId)} test={testResult[g.groupId]} />
                ))}
              </ul>
            ) : (
              <p className="mb-2 text-xs text-slate-500">Chưa có nhóm nào. Nhóm GMF tạo trong OA Manager tự xuất hiện ở đây (webhook <code>create_group</code>), hoặc dán link nhóm bên dưới.</p>
            )}
            <div className="flex flex-col gap-2 sm:flex-row">
              <input
                ...
                placeholder="Dán link chat nhóm (https://oa.zalo.me/chat?gid=…) hoặc ID nhóm"
              />
              <Button
                variant="secondary"
                loading={busy}
            ...
            <p className={`mt-1 text-xs ${numericOnly ? "text-rose-700" : "text-slate-500"}`}>
              {numericOnly
      ...
      {hasStaff && (
        <div className="mt-1">
          ...
          <div className="mt-1 flex flex-wrap gap-1">
            {depts.map((dp) => (
              <label key={dp.id} className={`cursor-pointer rounded-full border px-2 py-0.5 text-xs ${deptIds.includes(dp.id) ? "border-brand-600 bg-brand-50 text-brand-700" : "border-slate-200 text-slate-600"}`}>
      ...
      {dirty && (
        <div className="mt-2 flex gap-2">
          <Button
            size="sm"
            loading={busy}
            onClick={() => run(() => api(`/api/settings/zalo/groups/${encodeURIComponent(g.groupId)}`, { method: "PATCH", body: { categories: cats, departmentIds: deptIds } }), "Đã lưu loại tin của nhóm", onSaved)}
          >
            Lưu
          </Button>
