 ...
 * Ảnh của lần quét bị từ chối (không gắn log, không biết thuộc phòng nào) chỉ dành cho phạm vi toàn công ty có thêm `suspicious.view`. */
export const GET = handle<{ path: string[] }>(async (req, ctx) => {
  ...
  } else if (deptScope(u) !== null || !(await can(u, "suspicious.view"))) {
    throw forbidden();
  }
