...
- Chat bot tra cứu y khoa (v1.17.0, trả lời theo luồng từ v1.18.0): `/me/chatbot` + `src/lib/chatbot.ts`; gọi dự án medichat qua mạng docker kèm khóa `X-Chat-Key` (`/api/v1/chat/stream`, chữ hiện dần); cấp quyền theo phòng / từng người (`chatbot.grant`); không lưu nội dung, chỉ đếm lượt.
